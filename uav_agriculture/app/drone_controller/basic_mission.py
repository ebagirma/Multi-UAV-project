...
p = Planner(info['location_polygon'])
# p.pretty()
path = p.full_path()
# print(path)
...
if not connection_string:
    ...
    sitl_args = ['-I0', '--model', 'quad', '--home='+ str(info['home']['lat'])+',' + str(info['home']['lng']) + ',0,180']
    sitl.launch(sitl_args, await_ready=True, restart=True)
    # sitl = dronekit_sitl.start_default(info['home']['lat'], info['home']['lng'])
...
def adds_square_mission(aLocation, path):
    ...
    #Define the four MAV_CMD_NAV_WAYPOINT locations and add the commands
    for point in path:
        point1 = LocationGlobal(point[1], point[0], aLocation.alt)
        cmds.add(Command( 0, 0, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_WAYPOINT, 0, 0, 0, 0, 0, 0, point1.lat, point1.lon, 11))
