...
import json
import os
from collect_images import stream, stopStreaming          
...
def start_uav():
    ...
    def fly_robot(path):
        ...
        nextwaypoint = vehicle.commands.next
        initial = nextwaypoint
        # print(nextwaypoint)
        # vehicle = None
        # ct = CameraThread(vehicle)
        # ct.start()
    ...
    while True:
        ...
        with open("mission.json", "r") as f:
            try:
                chunkCommand = json.loads(''.join(f.readlines()))
            except ValueError:
                pass

        if not chunkCommand or "chunks" not in chunkCommand:
            time.sleep(5)
            continue
        
        curr_chunk = 0
        if "index" in chunkCommand:
            curr_chunk = chunkCommand["index"]

        chunks = chunkCommand["chunks"]        

        while True:
            if curr_chunk >= len(chunks):
                break
            if vehicle.battery.level > 85:
                fly_robot(chunks[curr_chunk])

                curr_chunk += 1
                chunkCommand["index"] += 1

                with open("mission.json", "w") as f:
                    f.write(json.dumps(chunkCommand))
            else:
                time.sleep(1)
